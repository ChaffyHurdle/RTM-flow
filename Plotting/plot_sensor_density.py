...
if __name__ == '__main__':
    ...
    p_fwd = np.array(loadmat('../Case1/meshes_sensors/p_fwd.mat')['p_fwd'])
    e_fwd = np.array(loadmat('../Case1/meshes_sensors/e_fwd.mat')['e_fwd'])
    t_fwd = np.array(loadmat('../Case1/meshes_sensors/t_fwd.mat')['t_fwd'])
    p_inv = np.array(loadmat('../Case1/meshes_sensors/p_inv.mat')['p_inv'])
    e_inv = np.array(loadmat('../Case1/meshes_sensors/e_inv.mat')['e_inv'])
    t_inv = np.array(loadmat('../Case1/meshes_sensors/t_inv.mat')['t_inv'])

    # Load sensors and means/vars/timers
    sensor_locs = []
    for s in [3,5,7,9,15]:
        sensors = int(s**2)
        sensor_locs.append(loadmat('../Case1/meshes_sensors/sensor_locs_' + str(sensors) + '.mat')['sensor_locs'])
    ...
    [axes[i,0].set_xticks([0,0.5,1]) for i in range(1,len(sensor_locs)+1)]
    [axes[i,0].set_yticks([0,0.5,1]) for i in range(1,len(sensor_locs)+1)]
    axes[1,0].set_title('Sensors')

    # Plot means
    [axes[i,j+1].tripcolor(triang_inv, facecolors=means[i-1][:,j],alpha=1-vars[i-1][:,j]/0.25,antialiased=True,linewidth=0.1,edgecolors='face',vmin=global_min,vmax=global_max,cmap='turbo', shading='flat') for i in range(1,len(sensor_locs)+1) for j in range(5)]
    ...
    plt.savefig('../Case1/SensorDensity/Example2/sensor_density2.jpg', format='jpg', bbox_inches='tight',dpi=400)
    plt.show()
