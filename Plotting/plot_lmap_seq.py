...
sensor_path = '../Case1/meshes_sensors/'
...
if __name__ == '__main__':
    ...
    sensor_locs = loadmat(sensor_path + 'sensor_locs_25.mat')['sensor_locs']
    ...
    im2 = [axes[1,i+1].tripcolor(triang_inv, facecolors=lmap_vars[:,i],vmin=0,vmax=0.25,edgecolors='none',cmap='turbo', shading='flat') for i in range(5)]
    [axes[j,i+1].set_xticks([]) for i in range(5) for j in range(2)]
    [axes[j,i+1].set_yticks([]) for i in range(5) for j in range(2)]
    [axes[j,i+1].set_xlim([0,1]) for i in range(5) for j in range(2)]
    [axes[j,i+1].set_ylim([0,1]) for i in range(5) for j in range(2)]
