...
% Computes the moving boundary condition on the adjoint variable
...
kappa_t_elem = kappa_elem; % kappa on elements
bndry_cond = zeros(1,length(obj_data.mesh_class.nodes)); % kappa on nodes
...
if t >= 5 % ensures elements and thus normals exist
    ...
    % Find boundary elements active at t+1 but not at t
    ...
    % Assign kappa on new boundary elements using nearest neighbour
    ...
    for i = 1:length(new_active_elements)
        ...
        % [~,b] = sort(distances);
        % max_b = min(length(b),3);
        % kappa_t_elem(elem) = mean(kappa_elem(prev_elems(b(1:max_b))));
        kappa_t_elem(elem) = kappa_t_elem(nearest_ind);
    end    
    
    % Update kappa on elements
    ...
    grad_lambda_temp(new_active_elements,:) = grad_lambda(nearest_inds(new_active_elements),:);
    ...
    % Update kappa on boundary nodes
